...
/**
 * MirrorServiceTest — Unit tests for [MirrorService].
 *
 * WHY: [MirrorService] is the central coordinator. Bugs in its state aggregation
 ...
 *
 * HOW: [MirrorService] is an Android [Service] and requires Android APIs for
...
class MirrorServiceTest {
    ...
    fun `ACTION_START has package-qualified value`() {
        assertEquals("dev.imirror.receiver.action.START", MirrorService.ACTION_START)
    }

    @Test
    fun `ACTION_STOP has package-qualified value`() {
        assertEquals("dev.imirror.receiver.action.STOP", MirrorService.ACTION_STOP)
    }

    @Test
    fun `ACTION_RESTART has package-qualified value`() {
        assertEquals("dev.imirror.receiver.action.RESTART", MirrorService.ACTION_RESTART)
    }
    ...
    fun `NOTIFICATION_ID is positive`() {
        assertTrue(
            ...
            MirrorService.NOTIFICATION_ID > 0
        )
    }

    @Test
    fun `CHANNEL_ID is non-empty`() {
        assertTrue(MirrorService.CHANNEL_ID.isNotEmpty())
    }
    ...
    /** The exact mapping lambda from [MirrorService.startAirPlay] — tested as a pure function. */
    ...
    // wiring between MirrorService and MainActivity without needing Android.
