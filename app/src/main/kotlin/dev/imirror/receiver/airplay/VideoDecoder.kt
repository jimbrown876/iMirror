...
class VideoDecoder(private val outputSurface: Surface) {
    ...
    fun initialize(spsBytes: ByteArray, ppsBytes: ByteArray, width: Int, height: Int) {
        ...
        val format = MediaFormat.createVideoFormat(
            ...
            // ─── Low-latency tuning ──────────────────────────────────────────
            // Mirroring is interactive: a frame that arrives late is worth less than a
            // frame that arrives now. These three keys tell the decoder to optimise for
            // latency over throughput/smoothness.

            // KEY_LOW_LATENCY (API 30+) disables the decoder's internal reordering buffer.
            // H.264 output can normally lag input by several frames so B-frames can be
            // reordered. AirPlay mirroring streams are baseline/high profile WITHOUT
            // B-frames, so that buffer only ever adds delay. Typically saves 2-3 frames
            // (~33-50 ms at 60 fps).
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                setInteger(MediaFormat.KEY_LOW_LATENCY, 1)
            }

            // Realtime priority — 0 = realtime, 1 = best-effort. Tells the codec scheduler
            // this stream must not be starved by background transcoding work.
            setInteger(MediaFormat.KEY_PRIORITY, 0)

            // Hint the expected frame rate so the decoder provisions clocks for 60 fps
            // rather than defaulting to a conservative rate and stuttering on fast content.
