 ...
 *
 * The mirror video server ([dev.imirror.receiver.airplay.handshake.MirrorStreamServer]) and audio server
 * ([dev.imirror.receiver.airplay.handshake.AudioStreamServer]) write these volatile fields as they run;
 * [dev.imirror.receiver.ui.StreamingScreen] polls [summary] a few times a second to render the HUD. Plain
 * volatile ints keep the hot path allocation-free — no flows or locks on the per-frame path.
 *
 * [overlayEnabled] is set from [dev.imirror.receiver.settings.AppSettings.showDebugOverlay] when the AirPlay
